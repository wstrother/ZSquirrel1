...
from zs_src.geometry import Vector
...
class ContextManager:
    ...
    def load_layer(self, name):
        ...
        ld["layer"] = layer
        env.set_value(name, layer)

        if not ld.get("pause_menu"):
    ...
    def load_item(self, item_key, *args, **kwargs):
        env = self.environment
        d = self.items_dict[item_key]

        cls_name = d.get("class", item_key)
        cls = CONTEXT_DICT[cls_name]
        ...
        if self.populate_dict:
            for key in self.populate_dict:
                ...
                for item_d in items:
                    ...
                    item = self.load_item(item_key, *args, **item_d)
...
CONTEXT_DICT = {
    ...
    "Cursor Layer": lambda: CursorLayer(
        "Cursor Layer"),
    ...
    "Vector": lambda name, *args, **kwargs: Vector(
        name, *args, **kwargs),

    "Vector Grid": lambda *args, **kwargs: VectorGrid(
        "Vector Grid", *args, **kwargs),

    "Cursor": lambda *args, **kwargs: CursorSprite(
        "Cursor", *args, **kwargs),
...
COLLISIONS_DICT = {
    "wall_collisions": RegionLayer.smooth_sprite_collision_system,
